...
// ─── DYNAMIC STORAGE ──────────────────────────────────────────────────────────
const STORAGE_KEY="ahc_horses_storage";
...
export default function GeneticsAnalyticsDashboard(){
  ...
  const handleLogin=()=>{
    if(loginUser==="admin" && loginPass==="asasas12"){
      localStorage.setItem("ahc_auth","yes");
      setLoggedIn(true);
      setLoginError("");
    }else{
  ...
  const logout=()=>{
    localStorage.removeItem("ahc_auth");
    setLoggedIn(false);
  };
  ...
  const importExcel=(e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    reader.onload=(evt)=>{
      try{
        ...
        const result:Horse[]=json.map((r:any)=>({
          id:mkId(),
          name:r.name||"",
          nameAr:r.nameAr||"",
          age:Number(r.age||1),
          color:(r.color||"grey") as Color,
          sex:(r.sex||"filly") as Sex,
          sire:r.sire||"",
          dam:r.dam||"",
          strain:(r.strain||"Dahman Shahwan") as Strain,
          family:(r.family||"Ansata Meryta") as Family,
          health:typeof r.health==="string"
            ?r.health.split(",").map((x:string)=>x.trim()).filter(Boolean) as Health[]
            :[],
        }));
        setHorses(result);
        alert(`Imported ${result.length} horses successfully`);
      }catch(err){
        console.error(err);
  ...
  if(!loggedIn){
    return(
      <div className="min-h-screen flex items-center justify-center p-6" style={{background:"#06071A",fontFamily:"Inter, Cairo, sans-serif"}}>
        <div className="w-full max-w-md rounded-[2rem] p-8" style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(201,169,110,0.2)",backdropFilter:"blur(20px)"}}>
          <div className="text-center mb-8">
            <h1 className="text-4xl font-black mb-2" style={{color:"#C9A96E"}}>Arabian Horse Center</h1>
            <p style={{color:"rgba(255,255,255,0.45)"}}>Admin Login</p>
          </div>
          <div className="space-y-4">
            ...
            {loginError&&<div className="text-sm font-bold text-center" style={{color:"#fb7185"}}>{loginError}</div>}
            <button onClick={handleLogin} className="w-full py-3 rounded-2xl font-black text-lg transition-all"
              style={{background:"#C9A96E",color:"#06071A"}}>LOGIN</button>
          </div>
        </div>
      </div>
    );
      ...
      <nav className="fixed top-[2px] left-0 right-0 z-50 no-print">
        <div style={{background:"rgba(6,7,26,0.82)",backdropFilter:"blur(24px)",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
          <div className="max-w-[1440px] mx-auto px-4 flex items-center h-[50px] gap-2">
            ...
            <button onClick={logout} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold shrink-0"
        ...
        <section id="registry" className="py-12 border-b" style={{borderColor:"rgba(255,255,255,0.05)"}}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <div className="flex gap-2 flex-wrap shrink-0 no-print" style={{marginTop:"-24px"}}>
              ...
              <label className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold cursor-pointer"
          ...
          {S.tot===0?
            <div className="py-20 text-center">
              ...
              <p className="font-black text-xl mb-2 text-white">{isAr?"لا توجد خيل مسجلة":"No horses registered yet"}</p>
              <p className="text-sm mb-6" style={{color:"rgba(255,255,255,0.35)"}}>{isAr?"قم باستيراد ملف Excel أو أضف جواداً يدوياً":"Import an Excel file or add a horse manually"}</p>
            </div>
