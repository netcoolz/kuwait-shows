...
type Strain = string; // تم التعديل لتستقبل أي رسن
type Family = string; // تم التعديل لتستقبل أي عائلة
...
const T={
  en:{
    ...
    affectedHorses: "Affected Horses:", empty: "Empty"
  },
  ar:{
    badge:"مركز الجواد العربي • تقرير خيل ٢٠٢٦",
    titlePrefix:"تقرير خيل\n",
    title:"العمري\nوالصحي",
    ...
    affectedHorses: "الخيل المصابة:", empty: "لا يوجد خيل"
  },
} as const;

// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const STRAINS:string[]  = ["Dahman Shahwan","Koheilan Rodan","Saqlawi Jidran","Obeyan Om Jreis","Hadban Enzahi"];
const FAMILIES:string[] = ["Ansata Meryta","Latiefa","Ansata White Nile","Alimaar Abbeyyah","Ansata Sherrara","Authentic Nabeelah","Ghazala Al Rayyan","Bilqis EV","Haifaa Al Waab","El Thay Maniha","NK Lubna","Loubna Al Waab","Rababa Al Rayyan"];
const C_MAP:Record<Color,string>={grey:"#94a3b8",chestnut:"#b45309",bay:"#7f1d1d",black:"#374151"};

// دعم الأرسان الإضافية برمجياً
const getStrainColor = (strain: string, index: number) => {
  const S_CLR:Record<string,string>={"Dahman Shahwan":"#4f8ef7","Koheilan Rodan":"#a78bfa","Saqlawi Jidran":"#34d399","Obeyan Om Jreis":"#c9a96e","Hadban Enzahi":"#fb7185"};
  if (S_CLR[strain]) return S_CLR[strain];
  const colors = ["#60A5FA", "#A78BFA", "#34D399", "#D4AF37", "#F472B6", "#FBBF24", "#38BDF8", "#F43F5E"];
...
function normalizeFamily(family:string):Family{
  const f = family ? family.trim() : "Ansata Meryta";
  return f;
}
...
export default function GeneticsAnalyticsDashboard(){
  ...
  // إضافات جديدة للتحكم في تفاعلية الأقسام
  const [openAge, setOpenAge] = useState<number | null>(null);
  const [openStrain, setOpenStrain] = useState<string | null>(null);
  ...
  async function loadFarmHorses(farmId: string) {
    ...
    const formatted: Horse[] = data.map((h: any, index: number) => {
      const rawBirthDate = h.birth_date || h.birthDate;
      const rawNameAr = h.name_ar || h.nameAr;
      ...
      return {
        ...
        sex: getHorseSex(h.gender, rawBirthDate),
        sire: h.sire || "",
        dam: h.dam || "",
        strain: normalizeStrain(h.strain),
        family: normalizeFamily(h.family),
        health: mapHealthConditions(h.health, h.otherHealth)
      };
    });
  ...
  useEffect(() => {
    const fn = () => {
      ...
      const ids = ["overview", "genetics", "health-age", "diseases", "production", "siblings"]; // تحديث الترتيب
  ...
  const S = useMemo(() => {
    ...
    // تعديل لحفظ المصفوفة بدلاً من مجرد العدد لعرض أسماء الخيل
    const ages = AGE_BUCKETS.map(b => horses.filter(h => h.age >= b.min && h.age <= b.max));
    
    const col: Record<Color, number> = { grey: 0, chestnut: 0, bay: 0, black: 0 };
    horses.forEach(h => { if (col[h.color] !== undefined) col[h.color]++; });
    
    // تجميع مرن للأرسان المضافة والموجودة
    const strainsMap: Record<string, Horse[]> = {};
    horses.forEach(h => {
      const s = h.strain || "Unknown";
      if (!strainsMap[s]) strainsMap[s] = [];
      strainsMap[s].push(h);
    });
    const sortedStrains = Object.entries(strainsMap).sort((a,b) => b[1].length - a[1].length);
    
    // تجميع مرن للعائلات
    const famMap: Record<string, Horse[]> = {};
    horses.forEach(h => {
      const f = h.family || "Unknown";
      if (!famMap[f]) famMap[f] = [];
      famMap[f].push(h);
    });
    const sortedFamilies = Object.entries(famMap).sort((a,b) => b[1].length - a[1].length);
    
    // تجميع الأمراض والخيل المصابة
    const disMap: Record<Health, Horse[]> = { melanoma:[], laminitis:[], epilepsy:[], pigmentation_loss:[], fertility:[], guttural_pouch:[], ovarian_tumor:[] };
    horses.forEach(h => h.health.forEach(k => { if (disMap[k]) disMap[k].push(h); }));
    ...
    // حساب الأفراس المنتجة (إناث وأعمارها فوق 3)
    const prodMares = horses.filter(h => (h.sex === "mare" || h.sex === "filly") && h.age >= 4).length;

    return { tot, ages, col, sortedStrains, sortedFamilies, disMap, sibs, siresRanked, damsRanked, prodMares };
  }, [horses]);
  ...
  const navItems = [
    ...
    { id: "production", label: tr.navProd }, { id: "siblings", label: tr.navSiblings },
  ];
  ...
  return (<>
    ...
    <div dir={dir} style={{ background: BG }} className="min-h-screen text-white pb-24 overflow-x-hidden">
      ...
      {/* Nav */}
      <nav className="fixed top-[82px] left-0 right-0 z-50 no-print">
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-4 md:px-7 pt-[88px]">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start gap-10">
              <div className="flex-1">
                ...
                <h1 className="text-5xl md:text-[5.5rem] font-black leading-[1.05] mb-5 whitespace-pre-line"
                  ...
                  {tr.titlePrefix} {farm?.display_name || farm?.farm_name || tr.title}
                </h1>
              ...
              <div className="grid grid-cols-2 gap-3 lg:w-[290px] shrink-0">
                {[
                  ...
                  { icon: Dna, v: S.sortedStrains.length, label: tr.strains, hex: GOLD, pct: 50 },
          ...
          <Cd accent={GOLD} className="p-7 mb-6">
            <div className="flex flex-col xl:flex-row gap-8 items-center">
              <div className="shrink-0 flex flex-col items-center gap-5">
                <Donut items={S.sortedStrains.slice(0, 8).map((s, i) => ({ name: s[0], pct: S.tot > 0 ? (s[1].length / S.tot) * 100 : 0, hex: getStrainColor(s[0], i) }))} size={200} thick={38} />
              </div>
              <div className="flex-1 space-y-3 w-full max-h-[350px] overflow-y-auto pr-1">
                {S.sortedStrains.map(([strainName, horsesList], i) => {
                  const pct = S.tot > 0 ? (horsesList.length / S.tot) * 100 : 0; 
                  const hex = getStrainColor(strainName, i);
                  const isOpen = openStrain === strainName;
                  return (
                    <div key={i} className="rounded-xl transition-all" style={{ background: isOpen ? "rgba(255,255,255,0.03)" : "transparent", border: `1px solid ${isOpen ? "rgba(255,255,255,0.08)" : "transparent"}` }}>
                      <div className="flex items-center justify-between p-2 cursor-pointer hover:bg-white/5 rounded-lg" onClick={() => setOpenStrain(isOpen ? null : strainName)}>
                        <div className="flex items-center gap-2.5">
                          <ChevronDown className={`w-4 h-4 transition-transform ${isOpen ? "rotate-180" : ""}`} style={{ color: "rgba(255,255,255,0.5)" }} />
                          <span className="font-bold text-sm" style={{ color: "rgba(255,255,255,0.75)" }}>{strainName}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-xl font-black text-white">{pct.toFixed(1)}%</span>
                          <span className="text-sm font-black px-2.5 py-0.5 rounded-lg" style={{ background: `${hex}20`, color: hex, border: `1px solid ${hex}40` }}>{horsesList.length}</span>
                        </div>
                      </div>
                      <AnimatePresence>
                        {isOpen && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <div className="p-3 flex flex-wrap gap-2">
                              {horsesList.map(h => <span key={h.id} className="text-[11px] font-bold px-2 py-1 rounded-md" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "white" }}>{h.name}</span>)}
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                      {!isOpen && <div className="px-2 pb-2"><Bar pct={pct} hex={hex} delay={.3 + i * .08} h="h-3" /></div>}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <Cd accent={GOLD} className="lg:col-span-2 p-7">
              ...
              <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
                {S.sortedFamilies.map(([famName, horsesList], i) => {
                  const pct = S.tot > 0 ? (horsesList.length / S.tot * 100) : 0;
                  const isOpen = openFam === famName;
                  return (
                    <div key={i} className="rounded-xl transition-all" style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.05)" }}>
                      <div className="flex items-center justify-between p-3 cursor-pointer hover:bg-white/5" onClick={() => setOpenFam(isOpen ? null : famName)}>
                        <div className="flex-1 mr-4">
                          <div className="flex items-center gap-2 mb-1">
                            <ChevronDown className={`w-3 h-3 transition-transform ${isOpen ? "rotate-180" : ""}`} style={{ color: "rgba(255,255,255,0.5)" }} />
                            <div className="font-bold text-sm text-white">{famName}</div>
                          </div>
                          {!isOpen && <Bar pct={pct} hex={GOLD} h="h-1.5" delay={i * .03} />}
                        </div>
                        <div className="text-right ml-4 shrink-0">
                          <div className="text-lg font-black" style={{ color: GOLD }}>{pct.toFixed(1)}%</div>
                          <div className="text-[10px] font-bold" style={{ color: "rgba(255,255,255,0.25)" }}>{horsesList.length}</div>
                        </div>
                      </div>
                      <AnimatePresence>
                        {isOpen && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <div className="px-3 pb-3 border-t pt-3 flex flex-wrap gap-1.5" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                              {horsesList.map(h => <span key={h.id} className="text-[10px] font-bold px-2 py-0.5 rounded-lg" style={{ background: `${GOLD}15`, border: `1px solid ${GOLD}30`, color: GOLD }}>{h.name}</span>)}
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
            <Cd accent="#4f8ef7" className="xl:col-span-3 p-7">
              ...
              <div className="space-y-4">
                {AGE_BUCKETS.map((b, i) => {
                  const horsesInBucket = S.ages[i];
                  const cnt = horsesInBucket.length; 
                  const pct = S.tot > 0 ? (cnt / S.tot) * 100 : 0;
                  const isOpen = openAge === i;
                  return (<div key={i} className="rounded-xl transition-all" style={{ background: isOpen ? "rgba(255,255,255,0.03)" : "transparent", border: `1px solid ${isOpen ? "rgba(255,255,255,0.08)" : "transparent"}` }}>
                    <div className="flex items-center justify-between p-2 cursor-pointer hover:bg-white/5 rounded-lg" onClick={() => setOpenAge(isOpen ? null : i)}>
                      <div className="flex items-center gap-2.5">
                        <ChevronDown className={`w-4 h-4 transition-transform ${isOpen ? "rotate-180" : ""}`} style={{ color: "rgba(255,255,255,0.5)" }} />
                        <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: b.hex }} />
                        <span className="font-bold text-sm" style={{ color: "rgba(255,255,255,0.75)" }}>{tr.ageGroup[i]}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-lg font-black text-white">{pct.toFixed(1)}%</span>
                        <span className="text-xs font-black px-2.5 py-0.5 rounded-lg" style={{ background: "rgba(255,255,255,0.1)", color: "white" }}>{cnt}</span>
                      </div>
                    </div>
                    <AnimatePresence>
                      {isOpen && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                          <div className="p-3 flex flex-wrap gap-2">
                            {horsesInBucket.length > 0 ? horsesInBucket.map(h => <span key={h.id} className="text-[11px] font-bold px-2 py-1 rounded-md" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "white" }}>{h.name}</span>) : <span className="text-xs p-2 text-white/30">{tr.empty}</span>}
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                    {!isOpen && <div className="px-2 pb-2"><Bar pct={pct} hex={b.hex} delay={i * .07} h="h-2.5" /></div>}
                  </div>);
                })}
              </div>
            </Cd>
            <Cd accent="#fb7185" className="xl:col-span-2 p-7">
              ...
              <div className="space-y-3">
                {HKEYS.map((k, i) => {
                  const affectedHorses = S.disMap[k] || [];
                  const cnt = affectedHorses.length; 
                  const hx = sevClr(k);
                  return cnt > 0 ? (<div key={i}>
                    ...
                    <Bar pct={(cnt / Math.max(...HKEYS.map(k2 => S.disMap[k2]?.length || 0), 1)) * 100} hex={hx} delay={.1 + i * .07} h="h-1.5" />
                  </div>) : null;
                })}
              </div>
            </Cd>
        ...
        <section id="diseases" className="py-12 border-b" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {DISEASE_TRACE.map((trace, i) => {
              const affectedHorses = S.disMap[trace.key] || [];
              const cnt = affectedHorses.length;
              ...
              return (<motion.div key={i} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * .08 }}>
                <div className="rounded-[1.5rem] overflow-hidden cursor-pointer transition-all"
                  ...
                  <div className="p-5">
                    ...
                    <Bar pct={(cnt / Math.max((S.disMap.melanoma || []).length || 1, 1)) * 100} hex={hx} h="h-1" />
                  ...
                  <AnimatePresence>
                    {isOpen && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: .3 }} className="overflow-hidden">
                      <div className="px-5 pb-5 border-t pt-4" style={{ borderColor: "rgba(251,113,133,0.1)" }}>
                        ...
                        {"analysisAr" in trace && <div className="p-4 rounded-xl relative overflow-hidden mb-4" style={{ background: "rgba(251,113,133,0.07)", border: "1px solid rgba(251,113,133,0.12)" }}>
                        ...
                        {/* 🌟 إظهار أسماء الخيل المصابة 🌟 */}
                        {affectedHorses.length > 0 && (
                          <div className="pt-3 mt-3 border-t" style={{borderColor: "rgba(251,113,133,0.15)"}}>
                            <p className="text-[12px] font-bold mb-2 text-rose-300">{tr.affectedHorses}</p>
                            <div className="flex flex-wrap gap-2">
                              {affectedHorses.map(h => (
                                <span key={h.id} className="text-[11px] font-bold px-2.5 py-1 rounded-md" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "white" }}>
                                  {h.name}
                                </span>
                              ))}
                            </div>
        ...
        {/* ═══ PRODUCTION (نُقل ليصبح قبل الأشقاء بناءً على طلبك) ═══ */}
        <section id="production" className="py-12 border-b" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
          <SecHead n="04" icon={Layers} hex="#fbbf24" title={tr.secProd} />
        ...
        {/* ═══ SIBLINGS ═══ */}
        <section id="siblings" className="py-12">
        ...
        {/* Footer */}
        <footer className="py-12 flex flex-col sm:flex-row justify-between items-center gap-5 border-t mt-8" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
          {[{ label: tr.footerIdea, name: "ناصر", hex: GOLD }, { label: tr.footerDev, name: "أحمد الصالح", hex: "#4f8ef7" }].map((f, i) => (
            <Cd key={i} accent={f.hex} className="px-7 py-5 flex items-center gap-4 w-full sm:w-auto hover:scale-[1.02] transition-transform">
